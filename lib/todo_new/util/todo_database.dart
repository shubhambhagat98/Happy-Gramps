...
class DatabaseHelper {
  ...
  String colRawTime = "rawTime";
  String colRawDate = "rawDate";
  String colRawDatetime = "rawDateTime";
  ...
  Future<Database> initializeDatabase() async {
    ...
    String path = directory.path + "todo_task.db";
  ...
  void _createDb(Database db, int newVersion) async {
    await db.execute(
        'CREATE TABLE $taskTable ($colId INTEGER PRIMARY KEY AUTOINCREMENT, $colTask TEXT, $colDate TEXT, $colTime TEXT, $colStatus TEXT, $colRawDate TEXT, $colRawTime TEXT, $colRawDatetime TEXT)');
  }
  ...
  Future<List<Map<String, dynamic>>> getInCompleteTaskMapList() async {
    Database db = await this.database;
    var result = db.rawQuery('SELECT * FROM $taskTable where $colStatus = "" order by $colRawDate, $colRawTime ASC');
    //var result = db.query(taskTable, orderBy: '$colStatus, $colDate, $colTime');
    return result;
  }
  ...
  Future<List<Map<String, dynamic>>> getCompleteTaskMapList() async {
    Database db = await this.database;
    var result = db.rawQuery('SELECT * FROM $taskTable where $colStatus = "Task Completed" order by $colRawDate, $colRawTime ASC');
    //var result = db.query(taskTable, orderBy: '$colStatus, $colDate, $colTime');
    return result;
  }
